...
#
# Script: Calculate whether the difference in effect of occupation an the
...
#
# Start script: 16/04/2026 (edits, FAH: globalize for GitHub upload)
# 
################################################################################
...
# read in model 2
FGR11.2 <- fread("output/GoogleDrive/FGR11/2025-01-30_INTERVENE_Occupation_Coeffs_CoxPH_model2_FinnGenR11.txt", data.table=FALSE)
FGR11.2$Biobank <- "FinnGen"
#
UKB.2 <- fread("output/GoogleDrive/UKB/2026-02-17_UKBiobank_INTERVENE_Occupation_CoxPH_model2_Coeffs.txt",data.table=FALSE)
UKB.2$Biobank <- "UK Biobank"
#
GS.2 <- fread("output/GoogleDrive/GS/2026-02-27_GS_INTERVENE_Occupation_CoxPH_model2_Coeffs.txt",data.table=FALSE)
...
#
FEMA.2 <- fread("output/GoogleDrive/MetaAnalysis/2026-03-12_INTERVENE_Occupation_FEMetaAnalysis_FinnGenR11_UKB_GenScot_model2.csv",data.table=FALSE)
...
#
REMA.2 <- fread("output/GoogleDrive/MetaAnalysis/2026-03-12_INTERVENE_Occupation_REMetaAnalysis_FinnGenR11_UKB_GenScot_model2.csv",data.table=FALSE)
...
GS.1b <- GS.1b[-which(GS.1b$trait %in% c("T1D","RHEUMA_SEROPOS_OTH",
                                         "C3_MELANOMA_SKIN","G6_EPLEPSY")),]
GS.2 <- GS.2[-which(GS.2$trait %in% c("T1D","RHEUMA_SEROPOS_OTH",
                                      "C3_MELANOMA_SKIN","G6_EPLEPSY")),]
...
# "Colorectal Cancer","C3_MELANOMA_SKIN"
#
################################################################################

# fixed effect
FEMA.1a <- FEMA.1a[-which(FEMA.1a$Phenotype %in% c("I9_AF","C3_COLORECTAL","C3_MELANOMA_SKIN")),]
FEMA.1b <- FEMA.1b[-which(FEMA.1b$Phenotype %in% c("I9_AF","C3_COLORECTAL","C3_MELANOMA_SKIN")),]
FEMA.2 <- FEMA.2[-which(FEMA.2$Phenotype %in% c("I9_AF","C3_COLORECTAL","C3_MELANOMA_SKIN")),]

# random effect
REMA.1a <- REMA.1a[-which(REMA.1a$Phenotype %in% c("I9_AF","C3_COLORECTAL","C3_MELANOMA_SKIN")),]
REMA.1b <- REMA.1b[-which(REMA.1b$Phenotype %in% c("I9_AF","C3_COLORECTAL","C3_MELANOMA_SKIN")),]
REMA.2 <- REMA.2[-which(REMA.2$Phenotype %in% c("I9_AF","C3_COLORECTAL","C3_MELANOMA_SKIN")),]
...
model1 <- data.frame(trait = c(FGR11.1a$trait,FGR11.1b$trait,UKB.1a$trait,
                     ...
                     HR = c(FGR11.1a$`OccupationUpper-level_HR`,FGR11.1b$PRS_HR,UKB.1a$`OccupationUpper-level_HR`,
                            UKB.1b$PRS_HR,GS.1a$`OccupationUpper-level_HR`,GS.1b$PRS_HR,FEMA.1a$HR,FEMA.1b$HR),
                     ...
                     beta = c(FGR11.1a$`OccupationUpper-level_beta`,FGR11.1b$PRS_beta,UKB.1a$`OccupationUpper-level_beta`,
                              UKB.1b$PRS_beta,GS.1a$`OccupationUpper-level_beta`,GS.1b$PRS_beta,FEMA.1a$Beta,FEMA.1b$Beta),
                     se = c(FGR11.1a$`OccupationUpper-level_se`,FGR11.1b$PRS_se,UKB.1a$`OccupationUpper-level_se`,
                            UKB.1b$PRS_se,GS.1a$`OccupationUpper-level_se`,GS.1b$PRS_se,FEMA.1a$SE,FEMA.1b$SE),
                     Test = c(rep("Upper OCC",nrow(FGR11.1a)),rep("PRS",nrow(FGR11.1b)), 
                              rep("Upper OCC", nrow(UKB.1a)),rep("PRS",nrow(UKB.1b)), 
                              rep("Upper OCC",nrow(GS.1a)),rep("PRS",nrow(GS.1b)),
                              rep("Upper OCC",nrow(FEMA.1a)), rep("PRS",nrow(FEMA.1b))),
...
model1$Test <- factor(model1$Test, levels = c("Upper OCC","PRS"), labels = c("UpperLevel Occupation","PRS"))
# Biobank as factor
...
model2 <- data.frame(trait = c(rep(FGR11.2$trait,2),rep(UKB.2$trait,2),
                     ...
                     HR = c(FGR11.2$`OccupationUpper-level_HR`,FGR11.2$PRS_HR,UKB.2$`OccupationUpper-level_HR`,
                            UKB.2$PRS_HR,GS.2$`OccupationUpper-level_HR`,GS.2$PRS_HR,FEMA.2$HR),
                     lb = c(FGR11.2$`OccupationUpper-level_HR_lower95`,FGR11.2$PRS_HR_lower95,
                            UKB.2$`OccupationUpper-level_HR_lower95`,UKB.2$PRS_HR_lower95,
                            GS.2$`OccupationUpper-level_HR_lower95`,GS.2$PRS_HR_lower95,FEMA.2$Cineg),
                     ub = c(FGR11.2$`OccupationUpper-level_HR_upper95`,FGR11.2$PRS_HR_upper95,
                            UKB.2$`OccupationUpper-level_HR_upper95`,UKB.2$PRS_HR_upper95,
                            GS.2$`OccupationUpper-level_HR_upper95`,GS.2$PRS_HR_upper95,FEMA.2$Cipos),
                     beta = c(FGR11.2$`OccupationUpper-level_beta`,FGR11.2$PRS_beta,UKB.2$`OccupationUpper-level_beta`,
                              UKB.2$PRS_beta,GS.2$`OccupationUpper-level_beta`,GS.2$PRS_beta,FEMA.2$Beta),
                     se = c(FGR11.2$`OccupationUpper-level_se`,FGR11.2$PRS_se,UKB.2$`OccupationUpper-level_se`,
                            UKB.2$PRS_se,GS.2$`OccupationUpper-level_se`,GS.2$PRS_se,FEMA.2$SE),
                     Test = c(rep("Upper OCC",nrow(FGR11.2)), rep("PRS",nrow(FGR11.2)), 
                              rep("Upper OCC", nrow(UKB.2)), rep("PRS",nrow(UKB.2)), 
                              rep("Upper OCC", nrow(GS.2)), rep("PRS",nrow(GS.2)),
                              rep("Upper OCC",0.5*nrow(FEMA.2)), rep("PRS",0.5*nrow(FEMA.2))),
...
# OCC levels as factor to plot them in order of magnitude
model2$Test <- factor(model2$Test, levels = c("Upper OCC","PRS"), labels = c("UpperLevel Occupation","PRS"))
# Biobank as factor
...
df12.EA <- df12[which(df12$Test=="UpperLevel Occupation"),]
...
# Determine whether difference between PGS effect (beta) low and UpperLevel Occupation
...
plot12_diff_EA$Test <- "UpperLevel Occupation"
plot12_diff_PRS$Test <- "PRS"
plot12_diff <- rbind(plot12_diff_EA,plot12_diff_PRS)

# write to file
fwrite(plot12_diff, paste("output/EmploymentStatus/MetaAnalysis/FGR11_UKB_GS/model2/", as.character(Sys.Date()), 
                         "_INTERVENE_Occupation_FEMetaAnalysis_FinnGenR11_UKB_GenScot_model1vs2_Differences.csv",sep=""))
...
model1r <- data.frame(trait = c(FGR11.1a$trait,FGR11.1b$trait,UKB.1a$trait,
                     ...
                     HR = c(FGR11.1a$`OccupationUpper-level_HR`,FGR11.1b$PRS_HR,UKB.1a$`OccupationUpper-level_HR`,
                            UKB.1b$PRS_HR,GS.1a$`OccupationUpper-level_HR`,GS.1b$PRS_HR,REMA.1a$HR,REMA.1b$HR),
                     lb = c(FGR11.1a$`OccupationUpper-level_HR_lower95`,FGR11.1b$PRS_HR_lower95,
                            UKB.1a$`OccupationUpper-level_HR_lower95`,UKB.1b$PRS_HR_lower95,
                            GS.1a$`OccupationUpper-level_HR_lower95`,GS.1b$PRS_HR_lower95,
                     ...
                     beta = c(FGR11.1a$`OccupationUpper-level_beta`,FGR11.1b$PRS_beta,UKB.1a$`OccupationUpper-level_beta`,
                              UKB.1b$PRS_beta,GS.1a$`OccupationUpper-level_beta`,GS.1b$PRS_beta,REMA.1a$Beta,REMA.1b$Beta),
                     se = c(FGR11.1a$`OccupationUpper-level_se`,FGR11.1b$PRS_se,UKB.1a$`OccupationUpper-level_se`,
                            UKB.1b$PRS_se,GS.1a$`OccupationUpper-level_se`,GS.1b$PRS_se,REMA.1a$SE,REMA.1b$SE),
                     Test = c(rep("Upper OCC",nrow(FGR11.1a)),rep("PRS",nrow(FGR11.1b)), 
                              rep("Upper OCC", nrow(UKB.1a)),rep("PRS",nrow(UKB.1b)), 
                              rep("Upper OCC",nrow(GS.1a)),rep("PRS",nrow(GS.1b)),
                              rep("Upper OCC",nrow(REMA.1a)), rep("PRS",nrow(REMA.1b))),
                     Biobanks = c(FGR11.1a$Biobank,FGR11.1b$Biobank,UKB.1a$Biobank,
                                  UKB.1b$Biobank,GS.1a$Biobank,GS.1b$Biobank,
...
model1r$Test <- factor(model1r$Test, levels = c("Upper OCC","PRS"), labels = c("UpperLevel Occupation","PRS"))
# Biobank as factor
...
modelr2 <- data.frame(trait = c(rep(FGR11.2$trait,2),rep(UKB.2$trait,2),
                     ...
                     HR = c(FGR11.2$`OccupationUpper-level_HR`,FGR11.2$PRS_HR,UKB.2$`OccupationUpper-level_HR`,
                            UKB.2$PRS_HR,GS.2$`OccupationUpper-level_HR`,GS.2$PRS_HR,REMA.2$HR),
                     lb = c(FGR11.2$`OccupationUpper-level_HR_lower95`,FGR11.2$PRS_HR_lower95,
                            UKB.2$`OccupationUpper-level_HR_lower95`,UKB.2$PRS_HR_lower95,
                            GS.2$`OccupationUpper-level_HR_lower95`,GS.2$PRS_HR_lower95,REMA.2$Cineg),
                     ub = c(FGR11.2$`OccupationUpper-level_HR_upper95`,FGR11.2$PRS_HR_upper95,
                            UKB.2$`OccupationUpper-level_HR_upper95`,UKB.2$PRS_HR_upper95,
                            GS.2$`OccupationUpper-level_HR_upper95`,GS.2$PRS_HR_upper95,REMA.2$Cipos),
                     beta = c(FGR11.2$`OccupationUpper-level_beta`,FGR11.2$PRS_beta,UKB.2$`OccupationUpper-level_beta`,
                              UKB.2$PRS_beta,GS.2$`OccupationUpper-level_beta`,GS.2$PRS_beta,REMA.2$Beta),
                     se = c(FGR11.2$`OccupationUpper-level_se`,FGR11.2$PRS_se,UKB.2$`OccupationUpper-level_se`,
                            UKB.2$PRS_se,GS.2$`OccupationUpper-level_se`,GS.2$PRS_se,REMA.2$SE),
                     Test = c(rep("Upper OCC",nrow(FGR11.2)), rep("PRS",nrow(FGR11.2)), 
                              rep("Upper OCC", nrow(UKB.2)), rep("PRS",nrow(UKB.2)), 
                              rep("Upper OCC", nrow(GS.2)), rep("PRS",nrow(GS.2)),
                              rep("Upper OCC",0.5*nrow(REMA.2)), rep("PRS",0.5*nrow(REMA.2))),
                     Biobanks = c(rep(FGR11.2$Biobank,2),rep(UKB.2$Biobank,2),
...
# EA levels as factor to plot them in order of magnitude
modelr2$Test <- factor(modelr2$Test, levels = c("Upper OCC","PRS"), labels = c("UpperLevel Occupation","PRS"))
# Biobank as factor
...
# split by Test for calculation
df12r.EA <- df12r[which(df12r$Test=="UpperLevel Occupation"),]
...
# combine EA and PRS results
plot12r_diff_EA$Test <- "UpperLevel Occupation"
plot12r_diff_PRS$Test <- "PRS"
plot12r_diff <- rbind(plot12r_diff_EA,plot12r_diff_PRS)

# write to file
fwrite(plot12r_diff, paste("output/EmploymentStatus/MetaAnalysis/FGR11_UKB_GS/model2/", as.character(Sys.Date()), 
                          "_INTERVENE_Occupation_REMetaAnalysis_FinnGenR11_UKB_GenScot_model1vs2_Differences.csv",sep=""))
...
# fixed effect
drive_upload(media = "output/EmploymentStatus/MetaAnalysis/FGR11_UKB_GS/model2/2026-03-12_INTERVENE_Occupation_FEMetaAnalysis_FinnGenR11_UKB_GenScot_model1vs2_Differences.csv",
             path = as_id("1Wi0KDwGtnZoUclUgZwu7F_Dwj-6uvJYH"),
             name = "2026-03-12_INTERVENE_Occupation_FEMetaAnalysis_FinnGenR11_UKB_GenScot_model1vs2_Differences.csv",
             type = "spreadsheet")

# random effect
drive_upload(media = "output/EmploymentStatus/MetaAnalysis/FGR11_UKB_GS/model2/2026-03-12_INTERVENE_Occupation_REMetaAnalysis_FinnGenR11_UKB_GenScot_model1vs2_Differences.csv",
             path = as_id("1Wi0KDwGtnZoUclUgZwu7F_Dwj-6uvJYH"),
             name = "2026-03-12_INTERVENE_Occupation_REMetaAnalysis_FinnGenR11_UKB_GenScot_model1vs2_Differences.csv",
             type = "spreadsheet")
