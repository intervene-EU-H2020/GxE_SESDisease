...
#
# Script meta-analysis model 2: Run fixed-effects meta-analysis of Cox
...
# occupation), and include the trait-specific PGS, sex (except for prostate and
# breast cancer), birth decade and the first 10 genetic PCs as covariates across
# FinnGen data release 11 (FGR11), UK Biobank (UKB), and Generation Scotland
# (GS) (script inspired by the INTERVENE Flagship project scripts:
# https://github.com/intervene-EU-H2020/flagship)
#
# required input data: FGR11 + UKB + GS model 2 (from INTERVENE GxE_SESDisease
# GoogleDrive Folder)
#
# Last edits: 16/04/2026 (edits, FAH: gloabalize for Github upload)
# 
################################################################################
...
# create long format to include both Occupation and PRS effect
FGR11.2long <- data.frame(trait = c(rep(FGR11.2$trait,2)),
                          beta = c(FGR11.2$`OccupationUpper-level_beta`,FGR11.2$PRS_beta),
                          se = c(FGR11.2$`OccupationUpper-level_se`,FGR11.2$PRS_se),
                          Test = c(rep(c("Occupation","PRS"),each=nrow(FGR11.2))),
                          Biobank = c(rep(FGR11.2$Biobank,2)))
UKB.2long <- data.frame(trait = c(rep(UKB.2$trait,2)),
                        beta = c(UKB.2$`OccupationUpper-level_beta`,UKB.2$PRS_beta),
                        se = c(UKB.2$`OccupationUpper-level_se`,UKB.2$PRS_se),
                        Test = c(rep(c("Occupation","PRS"),each=nrow(UKB.2))),
                        Biobank = c(rep(UKB.2$Biobank,2)))
GS.2long <- data.frame(trait = c(rep(GS.2$trait,2)),
                        beta = c(GS.2$`OccupationUpper-level_beta`,GS.2$PRS_beta),
                        se = c(GS.2$`OccupationUpper-level_se`,GS.2$PRS_se),
                        Test = c(rep(c("Occupation","PRS"),each=nrow(GS.2))),
...
# write to file
fwrite(metaresults.2, paste("output/EmploymentStatus/MetaAnalysis/FGR11_UKB_GS/model2/", as.character(Sys.Date()), 
                           "_INTERVENE_Occupation_FEMetaAnalysis_FinnGenR11_UKB_GenScot_model2.csv",sep=""))
...
# write to file
fwrite(metaresultsr.2, paste("output/EmploymentStatus/MetaAnalysis/FGR11_UKB_GS/model2/", as.character(Sys.Date()), 
                            "_INTERVENE_Occupation_REMetaAnalysis_FinnGenR11_UKB_GenScot_model2.csv",sep=""))
...
drive_upload(media = "output/EmploymentStatus/MetaAnalysis/FGR11_UKB_GS/model2/2026-03-12_INTERVENE_Occupation_FEMetaAnalysis_FinnGenR11_UKB_GenScot_model2.csv",
             path = as_id("1Wi0KDwGtnZoUclUgZwu7F_Dwj-6uvJYH"),
             name = "2026-03-12_INTERVENE_Occupation_FEMetaAnalysis_FinnGenR11_UKB_GenScot_model2.csv",
             type = "spreadsheet")

# model 2 - fixed effects
drive_upload(media = "output/EmploymentStatus/MetaAnalysis/FGR11_UKB_GS/model2/2026-03-12_INTERVENE_Occupation_REMetaAnalysis_FinnGenR11_UKB_GenScot_model2.csv",
             path = as_id("1Wi0KDwGtnZoUclUgZwu7F_Dwj-6uvJYH"),
             name = "2026-03-12_INTERVENE_Occupation_REMetaAnalysis_FinnGenR11_UKB_GenScot_model2.csv",
             type = "spreadsheet")
