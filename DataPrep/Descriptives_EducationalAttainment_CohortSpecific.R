...
desc.table.EA2 <- function(filelist) {
  ...
  Ncontrols_lowEA <- sum(filelist$EA2[which(filelist[,15]==0)]=="low")
  Ncases_lowEA <- sum(filelist$EA2[which(filelist[,15]==1)]=="low")
  Ncontrols_highEA <- sum(filelist$EA2[which(filelist[,15]==0)]=="high")
  Ncases_highEA <- sum(filelist$EA2[which(filelist[,15]==1)]=="high")
  ...
  Ncontrols_females_lowEA <- c(paste0(sum(filelist$SEX[which(filelist[,15]==0 & filelist$EA2=="low")]==1), 
                                      " (", 
                                      round(prop.table(table(filelist$SEX[which(filelist[,15]==0 & filelist$EA2=="low")]))[2]*100,1), 
                                      "%)"))
  Ncases_females_lowEA <- c(paste0(sum(filelist$SEX[which(filelist[,15]==1 & filelist$EA2=="low")]==1), 
                                   " (", 
                                   round(prop.table(table(filelist$SEX[which(filelist[,15]==1 & filelist$EA2=="low")]))[2]*100,1), 
                                   "%)"))
  Ncontrols_females_highEA <- c(paste0(sum(filelist$SEX[which(filelist[,15]==0 & filelist$EA2=="high")]==1), 
                                       " (", 
                                       round(prop.table(table(filelist$SEX[which(filelist[,15]==0 & filelist$EA2=="high")]))[2]*100,1), 
                                       "%)"))
  Ncases_females_highEA <- c(paste0(sum(filelist$SEX[which(filelist[,15]==1 & filelist$EA2=="high")]==1), 
                                    " (", 
                                    round(prop.table(table(filelist$SEX[which(filelist[,15]==1 & filelist$EA2=="high")]))[2]*100,1), 
                                    "%)"))
...
desc.table.comb$Ncontrols_females_lowEA[which(desc.table.comb$trait=="C3_BREAST")] <- paste0(sum(INTERVENE.list[["C3_BREAST"]]$SEX[which(INTERVENE.list[["C3_BREAST"]][,15]==0 & INTERVENE.list[["C3_BREAST"]]$EA2=="low")]==1)," (100%)")
desc.table.comb$Ncases_females_lowEA[which(desc.table.comb$trait=="C3_BREAST")] <- paste0(sum(INTERVENE.list[["C3_BREAST"]]$SEX[which(INTERVENE.list[["C3_BREAST"]][,15]==1 & INTERVENE.list[["C3_BREAST"]]$EA2=="low")]==1), " (100%)")
desc.table.comb$Ncontrols_females_highEA[which(desc.table.comb$trait=="C3_BREAST")] <- paste0(sum(INTERVENE.list[["C3_BREAST"]]$SEX[which(INTERVENE.list[["C3_BREAST"]][,15]==0 & INTERVENE.list[["C3_BREAST"]]$EA2=="high")]==1)," (100%)")
desc.table.comb$Ncases_females_highEA[which(desc.table.comb$trait=="C3_BREAST")] <- paste0(sum(INTERVENE.list[["C3_BREAST"]]$SEX[which(INTERVENE.list[["C3_BREAST"]][,15]==1 & INTERVENE.list[["C3_BREAST"]]$EA2=="high")]==1), " (100%)")
...
write.table(desc.table.comb, file=paste0("[PathToOutputFolder/]",as.character(Sys.Date()),
                                         "_",Biobank,"_INTERVENE_EducationalAttainment_CohortSpecific_SampleDescriptives.txt"),
