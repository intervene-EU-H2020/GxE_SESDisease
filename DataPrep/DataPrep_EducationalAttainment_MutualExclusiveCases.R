...
# where EA is the socioeconomic index; only retain mutual exclusive cases
#
# Required input data: biobank-specific INTERVENE phenotype file and
# biobank-specific INTERVENE PGS files
#
# Last edits: 20/04/2026 (FAH, edits: last edits before GitHub update)
#
################################################################################
...
# vector with relevant (non-cancer) diseases (1.	Type I Diabetes = T1D; 3.	Type
# 2 Diabetes = T2D; 4.	Gout = GOUT; 5.	Rheumatoid Arthritis = RHEUMA_SEROPOS_OTH
# ; 7.	Atrial Fibrillation = I9_AF; 9.	Asthma = J10_ASTHMA; 10.	Coronary Heart
# Disease = I9_CHD; 11.	Hip Osteoarthritis = COX_ARTHROSIS; 12.	Knee
# Osteoarthritis = KNEE_ARTHROSIS; 15.	Major Depression = F5_DEPRESSIO;
# 17.	Epilepsy = G6_EPLEPSY; 18.	Appendicitis = K11_APPENDACUT; 19. Alcohol Use
# Disorder = AUD_SWEDISH)
non_cancers <- c("T1D", "T2D", "GOUT", "RHEUMA_SEROPOS_OTH", "I9_AF", "J10_ASTHMA",
                 "I9_CHD", "COX_ARTHROSIS", "KNEE_ARTHROSIS", "F5_DEPRESSIO",
                 "G6_EPLEPSY","K11_APPENDACUT", "AUD_SWEDISH")

# vector with relevant (specific cancers) diseases (2.	Prostate Cancer =
# C3_PROSTATE; 6.	Breast Cancer = C3_BREAST; 8.	Colorectal Cancer =
# C3_COLORECTAL; 13.	Skin Melanoma = C3_MELANOMA_SKIN; 14.	Lung Cancer =
# C3_BRONCHUS_LUNG)
specific_cancers <- c("C3_PROSTATE", "C3_BREAST", "C3_COLORECTAL", "C3_MELANOMA_SKIN",
                      "C3_BRONCHUS_LUNG")

# vector with any cancer (16.	All Cancers = C3_CANCER)
any_cancer <- c("C3_CANCER")

# rename INTERVENE list to match phenotype names
names(INTERVENE.list) <- c("T1D", "C3_PROSTATE", "T2D", "GOUT", "RHEUMA_SEROPOS_OTH", 
                           "C3_BREAST", "I9_AF", "C3_COLORECTAL", "J10_ASTHMA",
                           "I9_CHD", "COX_ARTHROSIS", "KNEE_ARTHROSIS", "C3_MELANOMA_SKIN",
                           "C3_BRONCHUS_LUNG", "F5_DEPRESSIO", "C3_CANCER",
                           "G6_EPLEPSY","K11_APPENDACUT", "AUD_SWEDISH")

# for non-cancers and any cancer remove any row with multiple diseases, for
# specific cancers remove any row with another specific cancer or a non-cancer
# disease
INTERVENE.list.cleaned <- lapply(names(INTERVENE.list), function(dname) {
  # subset
  df_sub <- INTERVENE.list[[dname]]
  
  # Identify the case column in this dataset
  case_col <- names(df_sub)[
    !(names(df_sub) %in% c("ID","SEX","DATE_OF_BIRTH","PC1","PC2","PC3","PC4",
                           "PC5","PC6","PC7","PC8","PC9","PC10","ANCESTRY",
                           "END_OF_FOLLOWUP","EA","batch","chip","BL_YEAR","COHORT")) &
      !grepl("_DATE$", names(df_sub)) &
      !grepl("prs", names(df_sub), ignore.case=TRUE)
  ]
  
  case_col <- case_col[1]  # ensure single
  
  # Start from full INTERVENE for overlap logic
  full <- INTERVENE_FG_R11
  
  if (case_col %in% specific_cancers) {
    
    keep_ids <- full$ID[
      rowSums(full[, setdiff(specific_cancers, case_col)], na.rm=TRUE) == 0 &
        rowSums(full[, non_cancers], na.rm=TRUE) == 0
    ]
    
  } else if (case_col == any_cancer) {
    
    keep_ids <- full$ID[
      rowSums(full[, specific_cancers], na.rm=TRUE) == 0 &
        rowSums(full[, non_cancers], na.rm=TRUE) == 0
    ]
    
  } else if (case_col %in% non_cancers) {
    
    keep_ids <- full$ID[
      rowSums(full[, specific_cancers], na.rm=TRUE) == 0 &
        full[, any_cancer] == 0 &
        rowSums(full[, setdiff(non_cancers, case_col)], na.rm=TRUE) == 0
    ]
  }
  
  # Filter the subdataset
  df_sub_clean <- df_sub[df_sub$ID %in% keep_ids, ]
...
# add names to list
names(INTERVENE.list.cleaned) <- names(INTERVENE.list)

# overwrite old list and remove cleaned to safe space
INTERVENE.list <- INTERVENE.list.cleaned
rm(INTERVENE.list.cleaned)
...
glm.PGS.res.tab$PRS <- c(unlist(lapply(INTERVENE.list, function(x) { names(x)[20] })))
...
save(INTERVENE.list, file = paste("[PathToOutputFolder/]", as.character(Sys.Date()),
                                  "_",Biobank,"_INTERVENE_EducationalAttainment_dat_NoMutCases.RData",sep = ""))
