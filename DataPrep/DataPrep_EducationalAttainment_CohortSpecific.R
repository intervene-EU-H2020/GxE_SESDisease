...
# median ISCED 1997 categories to determine new threshold
median(as.numeric(pheno$ISCED97),na.rm = TRUE) 

# function to create new dichtomized EA variable 
calc.EA <- function(dat) {
  # create empty variable
  dat$EA2 <- NA
  
  #assign low vs high EA
  dat$EA2[dat$ISCED97 %in% c("ISCED 1","ISCED 2","ISCED 3")] <- "low" #if median was ISCED 3; otherwise adjust
  dat$EA2[dat$ISCED97 %in% c("ISCED 4","ISCED 5","ISCED 6")] <- "high" #if median was ISCED3; otherwise adjust
  
  # convert EA to factor
  dat$EA2 <- factor(dat$EA2, levels = c("low","high"))
...
INTERVENE.list <- lapply(INTERVENE.list, calc.EA)
...
# "PC9","PC10","ANCESTRY","END_OF_FOLLOWUP","ISCED97", and "EA2" (should mostly
...
CC = names(INTERVENE[,c("ID","SEX","DATE_OF_BIRTH","PC1","PC2","PC3","PC4","PC5",
                        ...
                        "END_OF_FOLLOWUP","ISCED97","EA2","Colorectal_Cancer_prs")]) 
...
HIP = names(INTERVENE[,c("ID","SEX","DATE_OF_BIRTH","PC1","PC2","PC3","PC4","PC5",
                         ...
                         "END_OF_FOLLOWUP","ISCED97","EA2","Hip_Osteoarthritis_prs")]) 
KNEE = names(INTERVENE[,c("ID","SEX","DATE_OF_BIRTH","PC1","PC2","PC3","PC4","PC5",
                          ...
                          "END_OF_FOLLOWUP","ISCED97","EA2","Knee_Osteoarthritis_prs")]) 
SKIN = names(INTERVENE[,c("ID","SEX","DATE_OF_BIRTH","PC1","PC2","PC3","PC4","PC5",
                          ...
                          "END_OF_FOLLOWUP","ISCED97","EA2","Melanoma_prs")]) 
LC = names(INTERVENE[,c("ID","SEX","DATE_OF_BIRTH","PC1","PC2","PC3","PC4","PC5",
                        ...
                        "END_OF_FOLLOWUP","ISCED97","EA2","Lung_Cancer_prs")]) 
...
EPILEP = names(INTERVENE[,c("ID","SEX","DATE_OF_BIRTH","PC1","PC2","PC3","PC4","PC5",
                            ...
                            "END_OF_FOLLOWUP","ISCED97","EA2","Epilepsy_prs")])
...
save(INTERVENE.list, file = paste("[PathToOutputFolder/]", as.character(Sys.Date()),
                                  "_",Biobank,"_INTERVENE_EducationalAttainment_CohortSpecific_dat.RData",sep = ""))
