...
# where EA is the socioeconomic index including EA PGS
#
# Required input data: biobank-specific INTERVENE phenotype file and
# biobank-specific INTERVENE PGS files
#
# Last edits: 20/04/2026 (FAH, edits: last edits before GitHub upload)
#
################################################################################
...
# read in EA PGS
EAPGS <- fread("[PathToPRSFiles/NameEAPGSFile]",
               select = c("#FID","SCORE1_AVG"),data.table = F)

# rename #FID to ID in PGS file
names(EAPGS)[1] <- "ID"

# rename PGS column
names(EAPGS)[2] <- "EAPGS"

# Merge EA PGS to combined file
INTERVENE <- merge(INTERVEN, EAPGS, by = "ID", all.x = TRUE)

# remove PGS file to save space
rm(EAPGS)
...
# and the PRSs (names as downloaded), the EA PGS, also include the following
# variables (rename if required):
...
CC = names(INTERVENE[,c("ID","SEX","DATE_OF_BIRTH","PC1","PC2","PC3","PC4","PC5",
                        ...
                        "END_OF_FOLLOWUP","ISCED97","EA","Colorectal_Cancer_prs","EAPGS")]) 
...
HIP = names(INTERVENE[,c("ID","SEX","DATE_OF_BIRTH","PC1","PC2","PC3","PC4","PC5",
                         ...
                         "END_OF_FOLLOWUP","ISCED97","EA","Hip_Osteoarthritis_prs","EAPGS")]) 
KNEE = names(INTERVENE[,c("ID","SEX","DATE_OF_BIRTH","PC1","PC2","PC3","PC4","PC5",
                          ...
                          "END_OF_FOLLOWUP","ISCED97","EA","Knee_Osteoarthritis_prs","EAPGS")]) 
SKIN = names(INTERVENE[,c("ID","SEX","DATE_OF_BIRTH","PC1","PC2","PC3","PC4","PC5",
                          ...
                          "END_OF_FOLLOWUP","ISCED97","EA","Melanoma_prs","EAPGS")]) 
LC = names(INTERVENE[,c("ID","SEX","DATE_OF_BIRTH","PC1","PC2","PC3","PC4","PC5",
                        ...
                        "END_OF_FOLLOWUP","ISCED97","EA","Lung_Cancer_prs","EAPGS")]) 
...
EPILEP = names(INTERVENE[,c("ID","SEX","DATE_OF_BIRTH","PC1","PC2","PC3","PC4","PC5",
                            ...
                            "END_OF_FOLLOWUP","ISCED97","EA","Epilepsy_prs","EAPGS")])
...
# run logistic regression analyses and obtain estimate of the EA PGS
glm.PGS.res <- foreach(i=1:length(INTERVENE.list)) %dopar% {
  summary(glm(as.formula(paste0(names(INTERVENE.list[[i]][18]), " ~ ",
                                "scale(", names(INTERVENE.list[[i]][24]),")",
                                " + SEX + AGE + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10")),
              family = binomial, data = INTERVENE.list[[i]]))$coefficients[2,]
}

# combine results in 1 data frame
glm.PGS.res.tab <- as.data.frame(matrix(NA, ncol = 0, nrow = 19)) # create empty dataframe with 19 rows
glm.PGS.res.tab$Trait <- c(unlist(lapply(INTERVENE.list, function(x) { names(x)[15] })))
glm.PGS.res.tab$PRS <- c(unlist(lapply(INTERVENE.list, function(x) { names(x)[24] })))
glm.PGS.res.tab$Estimate <- c(unlist(lapply(glm.PGS.res, function(x) { x[1] })))
glm.PGS.res.tab$SE <- c(unlist(lapply(glm.PGS.res, function(x) { x[2] })))
glm.PGS.res.tab$P <- c(unlist(lapply(glm.PGS.res, function(x) { x[4] })))
...
# this is not the case a strand flippage likely occurred and the PGS needs to be
# recoded.
neg.EA <- as.numeric(rownames(glm.PGS.res.tab[which(glm.PGS.res.tab$Estimate<0),])) # vector with the row numbers of the PGSs with negative associations with their phenotype

# recode PGS
for (i in 1:19) {
  if(i %in% neg.EA) {
    INTERVENE.list[[i]][24] <- INTERVENE.list[[i]][24]*-1
  }
}

# remove list with regression output to conserve memory
rm(glm.PGS.res)

# scale EA PGS
for (i in 1:length(INTERVENE.list)) {
  INTERVENE.list[[i]][,24] <- scale(INTERVENE.list[[i]][,24])
}
...
save(INTERVENE.list, file = paste("[PathToOutputFolder/]", as.character(Sys.Date()),
                                  "_",Biobank,"_INTERVENE_EducationalAttainment_dat_EAPGS.RData",sep = ""))
