...
#
# Authors: F.A. Hagenbeek [FAH] (fiona.hagenbeek@helsinki.fi); Max Tamlander
# [MT] (max.tamlander@helsinki.fi)
#
# Script: Bootstrapping lifetime risk estimates (based on:
# https://github.com/intervene-EU-H2020/flagship/blob/main/AbsoluteRiskEstimation/04_LifetimeRiskEstimation_Bootstrap.R)
#
# Data: Model 6 (PGS as factor: <20%, 20-40%, 40-60% (=reference group), 60-95%, >95%)
# stratified by EA for Finngen (FGR11)).
#
# Start script: 19/02/2025 
# Last edits: 24/06/2025 (FAH, edits: final checks prior to upload GitHub)
# 
################################################################################
...
# read in model 6
FGR11.6 <- fread("output/2classEA/FG11/CoxPropHaz_model6/2025-03-14_INTERVENE_SESDiffDiseases_Coeffs_CoxPH_model6_FinnGenR11.txt", data.table=FALSE)

# split by low vs high EA and reorganize hazard ratio files only need to keep c("Phenotype", "Group",
# "Beta", "SE","HR") where group reflect the HR for EA in the three PGS groups
# Low EA
low <- FGR11.6[which(FGR11.6$Test=="LowEA"),] # extract rows for low EA
FGR11.low <- data.frame(trait = rep(low$trait,4), # retain only required columns
                        ...
                        Group = c(rep("<20%",9),rep("20-40%",9),rep("60-95%",9),rep(">95%",9)))
# High EA
high <- FGR11.6[which(FGR11.6$Test=="HighEA"),] # extract rows for high EA
FGR11.high <- data.frame(trait = rep(high$trait,4), # retain only required columns
                         ...
                         Group = c(rep("<20%",9),rep("20-40%",9),rep("60-95%",9),rep(">95%",9)))
# combine in list
EDU <- list(LowEducation = FGR11.low,
            HighEducation = FGR11.high)
...
countries <- c("Finland")
biobank <- c("FinnGen")
education <- c("LowEducation","HighEducation")

# define which traits to analyse per biobank
biobank_traits <- list(LowEducation = unique(FGR11.low$trait),
                       HighEducation = unique(FGR11.high$trait))

# define which matching traits should be pulled from incidence/mortality/prevalence files
GBD_traits <- list(LowEducation = c("Diabetes mellitus type 2","Atrial fibrillation and flutter",
                                    "Asthma","Ischemic heart disease","Osteoarthritis hip",
                                    "Osteoarthritis knee","Total cancers",
                                    "Diabetes mellitus type 1","Prostate cancer"),
                   HighEducation = c("Total cancers","Diabetes mellitus type 1",
                                     "Diabetes mellitus type 2",
                                     "Atrial fibrillation and flutter","Asthma",
                                     "Ischemic heart disease","Osteoarthritis hip",
                                     "Osteoarthritis knee","Prostate cancer"))
...
for(j in 1:length(EDU)){
  ...
  print(education[j])
    ...
    for (z in 1:length(biobank_traits[[j]])) {  
      ...
      print(paste("Processing", biobank_traits[[j]][z], "for", education[j]))
    ...
    while(m < 1000){
      ...
      # Exract the hazard ratios and allocate to variables
      hazrats <- EDU[[j]]
      
      hazrats <- subset(hazrats, trait==biobank_traits[[j]][z])
      
      #Hazard Ratios
      hr01 <- with(hazrats[hazrats$Group=="<20%",], exp(rnorm(nrow(hazrats[hazrats$Group=="<20%",]), 
                                                                       hazrats[hazrats$Group=="<20%","Beta"], 
                                                                       hazrats[hazrats$Group=="<20%","SE"])))
      hr02 <- with(hazrats[hazrats$Group=="20-40%",], exp(rnorm(nrow(hazrats[hazrats$Group=="20-40%",]), 
                                                              hazrats[hazrats$Group=="20-40%","Beta"], 
                                                              hazrats[hazrats$Group=="20-40%","SE"])))
      hr04 <- with(hazrats[hazrats$Group=="60-95%",], exp(rnorm(nrow(hazrats[hazrats$Group=="60-95%",]), 
                                                              hazrats[hazrats$Group=="60-95%","Beta"], 
                                                              hazrats[hazrats$Group=="60-95%","SE"])))
      hr05 <- with(hazrats[hazrats$Group==">95%",], exp(rnorm(nrow(hazrats[hazrats$Group==">95%",]), 
                                                                        hazrats[hazrats$Group==">95%","Beta"], 
                                                                        hazrats[hazrats$Group==">95%","SE"])))
    ...
    bootstrapped_lifetimerisk <- data.frame(Age=rep(c("1-4 years","5-9 years","10-14 years","15-19 years","20-24 years","25-29 years","30-34 years","35-39 years","40-44 years","45-49 years","50-54 years","55-59 years","60-64 years","65-69 years","70-74 years","75-79 years"),5), 
                                            Group=c(rep("<20%",16),rep("20-40%",16),rep("40-60%",16),rep("60-95%",16), rep(">95%",16))) 
    ...
    # first line reads in precalculated lifetime risk estimates; NOTE tweak code so it doesn't need to extract Sys.Date or manually add date on which those analyses were run!
    lifetimeriskactual <- fread(paste0("output/LifetimeRisk/model6/",biobank,"/","2025-03-25_",biobank_traits[[j]][z],"_LifetimeRisk_",education[j],"_",biobank,".csv"), select=c("LifetimeRisk"), data.table=FALSE)#as.character(Sys.Date()),biobank_traits[[j]][z],"_LifetimeRisk_",biobank[j],".csv"), select=c("LifetimeRisk"), data.table=FALSE)
    ...
    bootstrapped_lifetimerisk$age <- rep(c(5,10,15,20,25,30,35,40,45,50,55,60,65,70,75,80),5)
    bootstrapped_lifetimerisk$Group <- factor(bootstrapped_lifetimerisk$Group, levels=c(c(">95%","60-95%","40-60%","20-40%","<20%")))
    
    write.csv(bootstrapped_lifetimerisk, paste0("output/LifetimeRisk/model6/",biobank,"/",as.character(Sys.Date()),"_",biobank_traits[[j]][z],"_LifetimeRisk_",education[j],"_Bootstrapped_",biobank,".csv"))
    ...
    ggsave(paste0("output/LifetimeRisk/model6/",biobank,"/",as.character(Sys.Date()),"_",biobank_traits[[j]][z],"_LifetimeRisk_",education[j],"_Bootstrapped_",biobank,".png"), height=10 , width=10)
