...
#
# Script model 4: Run logistic regression models with educational attainment
# (EA), trait-specific PGS, EA * trait-specific PGS interaction, sex (except for
# prostate and breast cancer), first 10 genetics PCs + birth year as covariates
#
...
# and prostate cancer), the first 10 genetic PCS, birth year, EA, and a EA by
# PGS interaction term as covariates use EA as factor where the low EA will
# become the offset
#
################################################################################

# run logistic regrssion in loop with foreach  in parallel for each of the 19 diseases.
res.glm.model4.by <- foreach(i=1:length(INTERVENE.list.80)) %dopar% {
  if(i==2 | i==6) {
    summary(glm(as.formula(paste0(names(INTERVENE.list.80[[i]][15]), 
                                  " ~ ", names(INTERVENE.list.80[[i]][19]), " + ",
                                  names(INTERVENE.list.80[[i]][19]),
                                  ":EA + EA + birth_year + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10")),
                family = binomial, data = INTERVENE.list.80[[i]]))
  } else {
    summary(glm(as.formula(paste0(names(INTERVENE.list.80[[i]][15]), 
                                  " ~ ",names(INTERVENE.list.80[[i]][19]), " + ",
                                  names(INTERVENE.list.80[[i]][19]),
                                  ":EA + EA + SEX + birth_year + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10")),
                family = binomial, data = INTERVENE.list.80[[i]]))
  }
}

# add trait names to list items 
names(res.glm.model4.by) <- c(unlist(lapply(INTERVENE.list.80, function(x) { names(x)[15] })))
...
modcoeffs.glm.model4.by.sex <- extract.coeffs(model.output = res.glm.model4.by[!names(res.glm.model4.by) %in% c("C3_PROSTATE","C3_BREAST")],
...
modcoeffs.glm.model4.by.nosex <- extract.coeffs(model.output = res.glm.model4.by[c("C3_PROSTATE","C3_BREAST")],
                                                filelist = INTERVENE.list.80[c("C3_PROSTATE","C3_BREAST")])
#combine into single result data frame
names(modcoeffs.glm.model4.by.sex) <- gsub(".*prs","PRS", names(modcoeffs.glm.model4.by.sex)) #rename prs variables columns
names(modcoeffs.glm.model4.by.nosex) <- gsub(".*prs","PRS", names(modcoeffs.glm.model4.by.nosex)) #rename prs variables columns
modcoeffs.glm.model4.by <- rbind.fill(modcoeffs.glm.model4.by.sex,modcoeffs.glm.model4.by.nosex)

# write table with model coefficients to output as tab-delimited text files
write.table(modcoeffs.glm.model4.by, file=paste0("[PathToOutputFolder/]",as.character(Sys.Date()),
                                                 "_",Biobank,"_INTERVENE_EducationalAttainment_LogReg_model4_Coeffs.txt"),
