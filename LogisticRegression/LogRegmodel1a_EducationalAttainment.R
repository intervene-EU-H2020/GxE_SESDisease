...
#
# Script model 1a: Run logistic regression models where EA is dichotomized into
# low vs high EA (reference = low EA), and include sex (except for prostate and
# breast cancer), birth year and the first 5 genetic PCs as covariates
#
# Required input data: biobank-specific INTERVENE combined phenotype and PGS
# file (80% training set)
#
...
###############################################################################
#
# Run logistic regression models with sex (except for breast and prostate
# cancer), the first 5 genetic PCS, birth year, and EA as covariates use EA as
...
res.glm.model1a.by <- foreach(i=1:length(INTERVENE.list.80)) %dopar% {
  if(i==2 | i==6) {
    summary(glm(as.formula(paste0(names(INTERVENE.list.80[[i]][15]), 
                                  " ~ EA + birth_year + PC1 + PC2 + PC3 + PC4 + PC5")),
                family = binomial, data = INTERVENE.list.80[[i]]))
  } else {
    summary(glm(as.formula(paste0(names(INTERVENE.list.80[[i]][15]), 
                                  " ~ EA + SEX + birth_year + PC1 + PC2 + PC3 + PC4 + PC5")),
                family = binomial, data = INTERVENE.list.80[[i]]))
  }
}

# add trait names to list items 
names(res.glm.model1a.by) <- c(unlist(lapply(INTERVENE.list.80, function(x) { names(x)[15] })))
...
modcoeffs.glm.model1a.by.sex <- extract.coeffs(model.output = res.glm.model1a.by[!names(res.glm.model1a.by) %in% c("C3_PROSTATE","C3_BREAST")],
...
modcoeffs.glm.model1a.by.nosex <- extract.coeffs(model.output = res.glm.model1a.by[c("C3_PROSTATE","C3_BREAST")],
                                                 filelist = INTERVENE.list.80[c("C3_PROSTATE","C3_BREAST")])
#combine into single result data frame
modcoeffs.glm.model1a.by <- rbind.fill(modcoeffs.glm.model1a.by.sex,modcoeffs.glm.model1a.by.nosex)

# write table with model coefficients to output as tab-delimited text files
write.table(modcoeffs.glm.model1a.by, file=paste0("[PathToOutputFolder/]",as.character(Sys.Date()),
                                                  "_",Biobank,"_INTERVENE_EducationalAttainment_LogReg_model1a_Coeffs.txt"),
