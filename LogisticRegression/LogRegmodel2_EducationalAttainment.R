...
#
# Script model 2: Run logistic regression models, where EA is dichotomized into
# low vs high EA (reference = low EA), and include the trait-specific PGS, sex
# (except for prostate and breast cancer), birth year and the first 10 genetic
# PCs as covariates
#
...
# and prostate cancer), birth year, the first 10 genetic PCS, and EA as
# covariates use EA as factor where the low EA will become the offset
#
################################################################################

# run logistic regrssion in loop with foreach  in parallel for each of the 19 diseases.
res.glm.model2.by <- foreach(i=1:length(INTERVENE.list.80)) %dopar% {
  if(i==2 | i==6) {
    summary(glm(as.formula(paste0(names(INTERVENE.list.80[[i]][15]), 
                                  " ~ ", names(INTERVENE.list.80[[i]][19]),
                                  " + EA + birth_year + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10")),
                family = binomial, data = INTERVENE.list.80[[i]]))
  } else {
    summary(glm(as.formula(paste0(names(INTERVENE.list.80[[i]][15]), 
                                  " ~ ",names(INTERVENE.list.80[[i]][19]), 
                                  " + EA + SEX + birth_year + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10")),
                family = binomial, data = INTERVENE.list.80[[i]]))
  }
}

# add trait names to list items 
names(res.glm.model2.by) <- c(unlist(lapply(INTERVENE.list.80, function(x) { names(x)[15] })))
...
modcoeffs.glm.model2.by.sex <- extract.coeffs(model.output = res.glm.model2.by[!names(res.glm.model2.by) %in% c("C3_PROSTATE","C3_BREAST")],
...
modcoeffs.glm.model2.by.nosex <- extract.coeffs(model.output = res.glm.model2.by[c("C3_PROSTATE","C3_BREAST")],
                                                 filelist = INTERVENE.list.80[c("C3_PROSTATE","C3_BREAST")])
#combine into single result data frame
names(modcoeffs.glm.model2.by.sex) <- gsub(".*prs","PRS", names(modcoeffs.glm.model2.by.sex)) #rename prs variables columns
names(modcoeffs.glm.model2.by.nosex) <- gsub(".*prs","PRS", names(modcoeffs.glm.model2.by.nosex)) #rename prs variables columns
modcoeffs.glm.model2.by <- rbind.fill(modcoeffs.glm.model2.by.sex,modcoeffs.glm.model2.by.nosex)

# write table with model coefficients to output as tab-delimited text files
write.table(modcoeffs.glm.model2.by, file=paste0("[PathToOutputFolder/]",as.character(Sys.Date()),
                                                 "_",Biobank,"_INTERVENE_EducationalAttainment_LogReg_model2_Coeffs.txt"),
