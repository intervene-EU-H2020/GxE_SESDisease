...
#
# Script model 0b: Run logistic regression models with sex (except for prostate
# and breast cancer), first 10 genetic PCs, and birth year as covariates.
#
# Required input data: biobank-specific INTERVENE combined phenotype and PGS
# file (80% training data)
#
...
# cancer), the first 10 genetic PCS, and birth year as covariates.
#
################################################################################

# run logistic regrssion in loop with foreach  in parallel for each of the 19 diseases.
res.glm.model0b.by <- foreach(i=1:length(INTERVENE.list.80)) %dopar% {
  if(i==2 | i==6) {
...
names(res.glm.model0b.by) <- c(unlist(lapply(INTERVENE.list.80, function(x) { names(x)[15] })))
...
modcoeffs.glm.model0b.by.sex <- extract.coeffs(model.output = res.glm.model0b.by[!names(res.glm.model0b.by) %in% c("C3_PROSTATE","C3_BREAST")],
...
modcoeffs.glm.model0b.by.nosex <- extract.coeffs(model.output = res.glm.model0b.by[c("C3_PROSTATE","C3_BREAST")],
                                                 filelist = INTERVENE.list.80[c("C3_PROSTATE","C3_BREAST")])
#combine into single result data frame
modcoeffs.glm.model0b.by <- rbind.fill(modcoeffs.glm.model0b.by.sex,modcoeffs.glm.model0b.by.nosex)

# write table with model coefficients to output as tab-delimited text files
write.table(modcoeffs.glm.model0b.by, file=paste0("[PathToOutputFolder/]",as.character(Sys.Date()),
                                                  "_",Biobank,"_INTERVENE_EducationalAttainment_LogReg_model0b_Coeffs.txt"),
