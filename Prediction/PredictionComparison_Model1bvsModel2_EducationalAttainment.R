...
FGR11.1b <- FGR11.1b[which(FGR11.1b$trait %in% trait),]
FGR11.2 <- FGR11.2[which(FGR11.2$trait %in% trait),]

# reorganize into long format
FGR11.1blong <- data.frame(trait = c(FGR11.1b$trait),
                           beta = c(FGR11.1b$PRS_beta),
                           se = c(FGR11.1b$PRS_se))
...
FGR11.1b.list <- split(FGR11.1blong,FGR11.1blong$trait)
FGR11.2.list <- split(FGR11.2long,FGR11.2long$trait)

# Compute risk scores from betas of Cox model from 80% FinnGen
INTERVENE.list_score_1b <- foreach(i=trait) %dopar% {
  exp(FGR11.1b.list[[i]]$beta * INTERVENE.list[[i]][20])
}
...
names(FGR11.1b.list) <- trait
names(INTERVENE.list_score_2) <- trait

# Normalize risk scores to range [0,1] (needed for NRI/IDI)
INTERVENE.list_score_1bn <- foreach(i=trait) %dopar% {
  (INTERVENE.list_score_1b[[i]] - min(INTERVENE.list_score_1b[[i]])) / (max(INTERVENE.list_score_1b[[i]]) - min(INTERVENE.list_score_1b[[i]]))
}
...
for (i in 1:length(INTERVENE.list_score_1bn)) {
  names(INTERVENE.list_score_1bn[[i]]) <- "score_1b"
}
...
INTERVENE.list <- foreach(i=1:length(INTERVENE.list)) %dopar% {
  cbind(INTERVENE.list[[i]],score_1b=INTERVENE.list_score_1bn[[i]])
}
INTERVENE.list <- foreach(i=1:length(INTERVENE.list)) %dopar% {
...
auc_1b <- foreach(i=1:length(INTERVENE.list)) %dopar% {
  roc(INTERVENE.list[[i]][,15], INTERVENE.list[[i]]$score_1b, ci = TRUE)
}
...
INTERVENE.list <- lapply(INTERVENE.list, function(df) {
  df %>%
  mutate(score_1b_rank = percent_rank(score_1b),
...
compute_nri_idi <- function(data_list, trait_names) {
  ...
  results_list <- lapply(seq_along(data_list), function(i) {
    ...
    x <- as.numeric(data$score_1b_rank)  # Reference model
