...
#
# Script: Compare prediction accuracy of model 0b (covariates only) with model
# 1b (covariates + main effect of disease-specific PGS) by comparing the models
# Receiver Operating Characteristic (ROC) Area Under the Curve(AUC), continuous
...
# read in model 0b
FGR11.0b.by <- fread("[PathToCoxOutputModel1a/CoxOutputModel0bFile]", data.table=FALSE)

# read in model 1b
FGR11.1b.by <- fread("[PathToCoxOutputModel1a/CoxOutputModel1bFile]", data.table=FALSE)
...
FGR11.0b.bylong <- data.frame(trait = c(rep(FGR11.0b.by$trait,2)),
                             beta = c(FGR11.0b.by$SEX_beta,
                                      FGR11.0b.by$birth_year_beta),
                             se = c(FGR11.0b.by$SEX_se,
                                    FGR11.0b.by$birth_year_se),
                             Test = c(rep("SEX",nrow(FGR11.0b.by)),rep("birth_year",nrow(FGR11.0b.by))))
...
FGR11.0b.by.list <- split(FGR11.0b.bylong,FGR11.0b.bylong$trait)
FGR11.1b.by.list <- split(FGR11.1b.bylong,FGR11.1b.bylong$trait)

# add trait names to list items
names(FGR11.0b.by.list) <- c(unlist(lapply(FGR11.0b.by.list, function(x) { x$trait[1] })))
...
INTERVENE.list_score_0b_by <- foreach(i=trait) %dopar% {
  exp(if (!is.na(FGR11.0b.by.list[[i]]$beta[which(FGR11.0b.by.list[[i]]$Test=="SEX")])) FGR11.0b.by.list[[i]]$beta[which(FGR11.0b.by.list[[i]]$Test=="SEX")] * (as.numeric(INTERVENE.list[[i]]$SEX)) else 0 + 
        FGR11.0b.by.list[[i]]$beta[which(FGR11.0b.by.list[[i]]$Test=="birth_year")] * INTERVENE.list[[i]]$birth_year)
}
...
# add trait names to list items 
names(INTERVENE.list_score_0b_by) <- trait
names(INTERVENE.list_score_1b_by) <- trait

# Normalize risk scores to range [0,1] (needed for NRI/IDI)
INTERVENE.list_score_0bn_by <- foreach(i=trait) %dopar% {
  (INTERVENE.list_score_0b_by[[i]] - min(INTERVENE.list_score_0b_by[[i]])) / (max(INTERVENE.list_score_0b_by[[i]]) - min(INTERVENE.list_score_0b_by[[i]]))
}
...
for (i in 1:length(INTERVENE.list_score_0bn_by)) {
  names(INTERVENE.list_score_0bn_by[[i]]) <- "score_0b_by"
}
...
INTERVENE.list <- foreach(i=1:length(INTERVENE.list)) %dopar% {
  cbind(INTERVENE.list[[i]],score_0b_by=INTERVENE.list_score_0bn_by[[i]])
}
INTERVENE.list <- foreach(i=1:length(INTERVENE.list)) %dopar% {
...
auc_0b_by <- foreach(i=1:length(INTERVENE.list)) %dopar% {
  if(names(INTERVENE.list[i]) %in% c("C3_CANCER","C3_MELANOMA_SKIN","KNEE_ARTHROSIS")) {
    roc(INTERVENE.list[[i]][,15], INTERVENE.list[[i]]$score_0b_by, ci = TRUE, direction=">")
  } else {
    roc(INTERVENE.list[[i]][,15], INTERVENE.list[[i]]$score_0b_by, ci = TRUE)
  }
}
...
auc_test0b1b_by <- foreach(i=1:length(INTERVENE.list)) %dopar% {
  roc.test(auc_0b_by[[i]], auc_1b_by[[i]])
}
...
# extract AUCs
auc0b1b_by_comparison_table <- extract_auc_comparison(roc_list = auc_test0b1b_by, trait_vector = trait)

# write table with model AUC to output as tab-delimited text files
write.table(auc0b1b_by_comparison_table, file=paste0("[PathToOutputFolder/]",as.character(Sys.Date()),
                                                  "_",Biobank,"_EUR_INTERVENE_EducationalAttainment_AUCcomparison_Model0b-1b_by.txt"),
...
INTERVENE.list <- lapply(INTERVENE.list, function(df) {
  df %>%
  mutate(score_0b_by_rank = percent_rank(score_0b_by),
         score_1b_by_rank = percent_rank(score_1b_by))
})
...
compute_nri_idi <- function(data_list, trait_names) {
  ...
  results_list <- lapply(seq_along(data_list), function(i) {
    ...
    x <- as.numeric(data$score_0b_by_rank)  # Reference model
    y <- as.numeric(data$score_1b_by_rank)  # New model
...
write.table(nri_idi_results, file=paste0("[PathToOutputFolder/]",as.character(Sys.Date()),
                                             "_",Biobank,"_EUR_INTERVENE_EducationalAttainment_NRI_IDI_Model0b-1b_by.txt"),
            row.names=F, col.names = T, sep="\t",quote = F)
