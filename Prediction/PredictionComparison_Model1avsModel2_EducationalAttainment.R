...
#   the output of the CoxPH models 1a and 2 based 80% of the 
...
#
# Last edits: 16/04/2026 (edits, FAH: Replace logistic regression models in 80% FG
# with CoxPH models in 80% FG)
#
################################################################################
...
auc_test1a2_by <- foreach(i=1:length(INTERVENE.list)) %dopar% {
  roc.test(auc_1a_by[[i]], auc_2_by[[i]])
}
...
# extract AUCs
auc1a2_by_comparison_table <- extract_auc_comparison(roc_list = auc_test1a2_by, trait_vector = trait)

# write table with model AUC to output as tab-delimited text files
write.table(auc1a2_by_comparison_table, file=paste0("[PathToOutputFolder/]",as.character(Sys.Date()),
...
INTERVENE.list <- lapply(INTERVENE.list, function(df) {
  df %>%
  mutate(score_1a_by_rank = percent_rank(score_1a_by),
...
compute_nri_idi <- function(data_list, trait_names) {
  ...
  results_list <- lapply(seq_along(data_list), function(i) {
    ...
    x <- as.numeric(data$score_1a_by_rank)  # Reference model
