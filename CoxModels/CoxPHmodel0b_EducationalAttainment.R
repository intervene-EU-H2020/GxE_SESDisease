...
#
# Script model 0b: Run Cox Proportional-Hazards models with age of onset as
# timescale, with sex (except for prostate and breast cancer), first 10 genetic
...
res.cox.model0b <- foreach(i=1:length(INTERVENE.list)) %dopar% {
  if(names(INTERVENE.list[[i]][15])=="C3_PROSTATE" | names(INTERVENE.list[[i]][15])=="C3_BREAST") {
    cox.model.PGS(filelist = INTERVENE.list[[i]],covformula = mod0bnosex.formula) 
  } else {
    cox.model.PGS(filelist = INTERVENE.list[[i]],covformula = mod0bsex.formula)
  }
}

# add trait names to list items 
names(res.cox.model0b) <- c(unlist(lapply(INTERVENE.list, function(x) { names(x)[15] })))
...
modcoeffs.cox.model0b.sex <- extract.coeffs(model.output = res.cox.model0b[!names(res.cox.model0b) %in% c("C3_PROSTATE","C3_BREAST")],
...
modcoeffs.cox.model0b.nosex <- extract.coeffs(model.output = res.cox.model0b[c("C3_PROSTATE","C3_BREAST")],
                                              filelist = INTERVENE.list[c("C3_PROSTATE","C3_BREAST")])
#combine into single result data frame
modcoeffs.cox.model0b <- rbind.fill(modcoeffs.cox.model0b.sex,modcoeffs.cox.model0b.nosex) #combine data

# write table with model coefficients to output as tab-delimited text files
write.table(modcoeffs.cox.model0b, file=paste0("[PathToOutputFolder/]",as.character(Sys.Date()),
                                               "_",Biobank,"_INTERVENE_EducationalAttainment_CoxPH_model0b_Coeffs_80percent.txt"),
