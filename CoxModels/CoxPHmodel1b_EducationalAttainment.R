...
#
# Script model 1b: Run Cox Proportional-Hazards models with age of onset as
# timescale, with trait-specifict polygenic score (PGS), sex (except for
# prostate and breast cancer), first 10 genetic PCs, and birth decade as
# covariates.
#
...
###############################################################################
#
# Run Cox Proportional-Hazards model 1b) trait-specific polygenic score (PGS),
...
res.cox.model1b <- foreach(i=1:length(INTERVENE.list)) %dopar% {
  if(names(INTERVENE.list[[i]][15])=="C3_PROSTATE" | names(INTERVENE.list[[i]][15])=="C3_BREAST") {
    cox.model.PGS(filelist = INTERVENE.list[[i]],covformula = mod1bnosex.formula) 
  } else {
    cox.model.PGS(filelist = INTERVENE.list[[i]],covformula = mod1bsex.formula)
  }
}

# add trait names to list items 
names(res.cox.model1b) <- c(unlist(lapply(INTERVENE.list, function(x) { names(x)[15] })))
...
modcoeffs.cox.model1b.sex <- extract.coeffs(model.output = res.cox.model1b[!names(res.cox.model1b) %in% c("C3_PROSTATE","C3_BREAST")],
...
modcoeffs.cox.model1b.nosex <- extract.coeffs(model.output = res.cox.model1b[c("C3_PROSTATE","C3_BREAST")],
                                              filelist = INTERVENE.list[c("C3_PROSTATE","C3_BREAST")])
#combine into single result data frame
names(modcoeffs.cox.model1b.sex) <- gsub("^.*\\prs","PRS", names(modcoeffs.cox.model1b.sex)) #rename prs variables columns
names(modcoeffs.cox.model1b.nosex) <- gsub("^.*\\prs","PRS", names(modcoeffs.cox.model1b.nosex)) #rename prs variables columns
modcoeffs.cox.model1b <- rbind.fill(modcoeffs.cox.model1b.sex,modcoeffs.cox.model1b.nosex) #combine data
...
write.table(modcoeffs.cox.model1b, file=paste0("[PathToOutputFolder/]",as.character(Sys.Date()),
                                               "_",Biobank,"_INTERVENE_EducationalAttainment_CoxPH_model1b_Coeffs.txt"),
