...
# covariates) using Cox Proportional Hazard Model (CoxPH) with all FG data and
# CoxPH models in FG data after excluding cases with disease-onset before entry
# into FG (i.e., before donating a blood sample)
#
# Data: Results CoxPH model 4 full FG + subset FG (excl. cases before cohort entry)
#
# Last edits: 20/04/2026 (edits, FAH: final edits before GitHub Upload)
# 
################################################################################
...
FGR11.sub <- fread("output/GoogleDrive/FGR11/2026-02-10_INTERVENE_SESDiffDiseases_Coeffs_CoxPH_model4_nocasesbeforeBlood_FinnGenR11.txt", data.table=FALSE)


################################################################################
#
# Combine full and subset results for plotting
#
################################################################################
...
model4$Sample <- factor(model4$Sample, levels = c("Full","Subset"),
                       labels = c("Full FinnGen",
                                  "Exclude cases before entry FinnGen"))
...
plot12_grouped_EA <- plot12.df.EA %>%
  ...
  reframe(
    ...
    Model2_HR = HR[Sample == "Exclude cases before entry FinnGen"],
    Model1_HR_lb = lb[Sample == "Full FinnGen"],
    Model2_HR_lb = lb[Sample == "Exclude cases before entry FinnGen"],
    Model1_HR_ub = ub[Sample == "Full FinnGen"],
    Model2_HR_ub = ub[Sample == "Exclude cases before entry FinnGen"],
    Model1b = beta[Sample == "Full FinnGen"],
    Model2b = beta[Sample == "Exclude cases before entry FinnGen"],
    Model1se = se[Sample == "Full FinnGen"],
    Model2se = se[Sample == "Exclude cases before entry FinnGen"],
...
fwrite(plot12_diff, paste("output/2classEA/FG11/CoxPropHaz_model4/", as.character(Sys.Date()),
                          "_INTERVENE_SESDiffDiseases_Differences_FullFinnGenR11vsNocasebeforeBlood_model4.csv",sep=""))
...
# model 1a
drive_upload(media = "output/2classEA/FG11/CoxPropHaz_model4/2026-02-25_INTERVENE_SESDiffDiseases_Differences_FullFinnGenR11vsSubset_model4.csv",
             path = as_id("18iI9QxxJ7WXrXO6hcNal_amvGmR1F_jv"),
             name = "2026-02-25_INTERVENE_SESDiffDiseases_Differences_FullFinnGenR11vsSubset_model4.csv",
             type = "spreadsheet")
