...
###############################################################################
#
# Run Cox Proportional-Hazards models stratified by occupation level,
...
mod6sex.formula <- paste0("PGS_group + SEX + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10")
mod6nosex.formula <- paste0("PGS_group + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10")

# run Cox-PH model 6 in loop with foreach in parallel for each diseases.
# run stratified by low occupation
res.cox.model6.low <- foreach(i=1:length(lowOCC)) %dopar% {
  if(names(lowOCC[[i]][15])=="C3_PROSTATE") {
    cox.model(filelist = lowOCC[[i]],covformula = mod6nosex.formula) 
  } else {
    cox.model(filelist = lowOCC[[i]],covformula = mod6sex.formula)
  }
}

# add trait names to list items 
names(res.cox.model6.low) <- c(unlist(lapply(lowOCC, function(x) { names(x)[15] })))

# run stratified by high occupation
res.cox.model6.high <- foreach(i=1:length(highOCC)) %dopar% {
  if(names(highOCC[[i]][15])=="C3_PROSTATE") {
    cox.model(filelist = highOCC[[i]],covformula = mod6nosex.formula) 
  } else {
    cox.model(filelist = highOCC[[i]],covformula = mod6sex.formula)
  }
}

# add trait names to list items 
names(res.cox.model6.high) <- c(unlist(lapply(highOCC, function(x) { names(x)[15] })))
...
modcoeffs.cox.model6.nosex.high <- extract.coeffs(model.output = res.cox.model6.high[c("C3_PROSTATE","C3_BREAST")],
                                                  filelist = highOCC[c("C3_PROSTATE","C3_BREAST")])
...
modcoeffs.cox.model6$Test <- c(rep("Lower-level",.5*nrow(modcoeffs.cox.model6)),rep("Upper-level",.5*nrow(modcoeffs.cox.model6)))
...
write.table(modcoeffs.cox.model6, file=paste0("[PathToOutputFolder/]",as.character(Sys.Date()),
                                              "_",Biobank,"_INTERVENE_Occupation_CoxPH_model6_Coeffs.txt"),
