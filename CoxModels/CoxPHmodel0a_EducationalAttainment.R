...
#
# Script model 0a: Run Cox Proportional-Hazards models with age of onset as
# timescale, and include sex (except for prostate and breast cancer), birth year
# and the first 5 genetic PCs as covariates
#
# Required input data: biobank-specific INTERVENE combined phenotype and PGS
# file
#
# Last edits: 16/04/2026 (FAH, edits: globalize script for use in other
...
###############################################################################
#
# Run Cox Proportional-Hazards model 0a with age at disease onset as timescale.
# Include sex (except for prostate and breast cancer), first 5 genetic PCs +
# birth year as covariates.
#
################################################################################
...
res.cox.model0a <- foreach(i=1:length(INTERVENE.list)) %dopar% {
  if(names(INTERVENE.list[[i]][15])=="C3_PROSTATE" | names(INTERVENE.list[[i]][15])=="C3_BREAST") {
    cox.model(filelist = INTERVENE.list[[i]],covformula = mod0anosex.formula) 
  } else {
    cox.model(filelist = INTERVENE.list[[i]],covformula = mod0asex.formula)
  }
}

# add trait names to list items 
names(res.cox.model0a) <- c(unlist(lapply(INTERVENE.list, function(x) { names(x)[15] })))
...
modcoeffs.cox.model0a.sex <- extract.coeffs(model.output = res.cox.model0a[!names(res.cox.model0a) %in% c("C3_PROSTATE","C3_BREAST")],
...
modcoeffs.cox.model0a.nosex <- extract.coeffs(model.output = res.cox.model0a[c("C3_PROSTATE","C3_BREAST")],
                                              filelist = INTERVENE.list[c("C3_PROSTATE","C3_BREAST")])
#combine into single result data frame
modcoeffs.cox.model0a <- rbind.fill(modcoeffs.cox.model0a.sex,modcoeffs.cox.model0a.nosex)

# write table with model coefficients to output as tab-delimited text files
write.table(modcoeffs.cox.model0a, file=paste0("[PathToOutputFolder/]",as.character(Sys.Date()),
                                               "_",Biobank,"_INTERVENE_EducationalAttainment_CoxPH_model0a_Coeffs_80percent.txt"),
