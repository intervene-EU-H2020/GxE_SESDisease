...
# timescale, EA, trait-specific PGS, EA * trait-specific PGS interaction, EA
# PGS, trait-specific PGS * EA PGS interaction, sex (except for protate and
# breast cancer), first 10 genetics PCs + birth decade as covariates
#
...
# disease and EA PGSs, EA by PGS interaction terms, and EA PGS by disease PGS
cox.model.GxG <- function(filelist,covformula) {
  fit <- coxph(as.formula(paste0("Surv(AGE,",names(filelist[15]),") ~ ",
                                 names(filelist[19])," + EAPGS + ",
                                 names(filelist[19]),":EA + ",
                                 names(filelist[19])," * EAPGS + ",
                                 covformula)), 
...
res.cox.model4 <- foreach(i=1:length(INTERVENE.list)) %dopar% {
  if(names(INTERVENE.list[[i]][15])=="C3_PROSTATE" | names(INTERVENE.list[[i]][15])=="C3_BREAST") {
    cox.model.GxG(filelist = INTERVENE.list[[i]],covformula = mod4nosex.formula) 
  } else {
    cox.model.GxG(filelist = INTERVENE.list[[i]],covformula = mod4sex.formula)
  }
}
...
write.table(modcoeffs.cox.model4, file=paste0("[PathToOutputFolder/]",as.character(Sys.Date()),
                                              "_",Biobank,"_INTERVENE_EducationalAttainment_CoxPH_model4_Coeffs_GxG.txt"),
