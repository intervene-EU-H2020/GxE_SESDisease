...
# breast and prostate cancer), the first 10 genetic PCS, Occupation, and an
# Occupation by PGS interaction term as covariates use Occupation as factor
# where the Manual worker will become the offset.
#
################################################################################

# function to run Cox Proportional Hazards Models with multiple covariates,
# PGSs, and Occupation status by PGS interaction terms
cox.model.GxOcc <- function(filelist,covformula) {
  fit <- coxph(as.formula(paste0("Surv(AGE,",names(filelist[15]),") ~ ",
                                 names(filelist[19])," + ",
                                 names(filelist[19]),":Occupation + ",
                                 covformula)), 
               data =  filelist, na.action = na.exclude)
  return(fit)
}

# create the formula with Occupation, the interaction term, the first 10 genetic
# PCs, and sex as covariates, and one with only Occupation, the interaction
# term, and the first 10 genetic PCs as covariates (for running the analyses for
# prostate and breast cancer)
mod4sex.formula <- paste0("Occupation  + SEX + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10")
mod4nosex.formula <- paste0("Occupation  + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10")
...
res.cox.model4 <- foreach(i=1:length(INTERVENE.list)) %dopar% {
  if(names(INTERVENE.list[[i]][15])=="C3_PROSTATE" | names(INTERVENE.list[[i]][15])=="C3_BREAST") {
    cox.model.GxOcc(filelist = INTERVENE.list[[i]],covformula = mod4nosex.formula)
  } else {
    cox.model.GxOcc(filelist = INTERVENE.list[[i]],covformula = mod4sex.formula)
  }
}
...
write.table(modcoeffs.cox.model4, file=paste0("[PathToOutputFolder/]",as.character(Sys.Date()),
                                              "_",Biobank,"_INTERVENE_Occupation_CoxPH_model4_Coeffs.txt"),
