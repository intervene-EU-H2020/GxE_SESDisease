...
# file
#
# Last edits: 17/02/2026 (FAH, edits: globalize script for use in other
...
# convert ISCED 97 factor into numeric
INTERVENE.list <- lapply(INTERVENE.list, function(df) {
  df$ISCED97 <- as.numeric(df$ISCED97)
  df
})
...
mod4sex.formula <- paste0("ISCED97 + SEX + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + birthdecade")
mod4nosex.formula <- paste0("ISCED97 + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + birthdecade")
...
write.table(modcoeffs.cox.model4, file=paste0("[PathToOutputFolder/]",as.character(Sys.Date()),
                                              "_",Biobank,"_INTERVENE_EducationalAttainment_allISCED_CoxPH_model4_Coeffs.txt"),
