...
# timescale, with trait-specific PGS, sex (except for breast and prostate
# cancer), birth decade, and the first 10 genetic PCS as covariates stratified
# by Education level.
#
...
# covariates stratified by Education level.
#
################################################################################
...
modcoeffs.cox.model3$Test <- c(rep("LowEA",.5*nrow(modcoeffs.cox.model3)),rep("HighEA",.5*nrow(modcoeffs.cox.model3)))
...
write.table(modcoeffs.cox.model3, file=paste0("[PathToOutputFolder/]",as.character(Sys.Date()),
                                              "_",Biobank,"_INTERVENE_EducationalAttainment_CohortSpecific_CoxPH_model3_Coeffs.txt"),
