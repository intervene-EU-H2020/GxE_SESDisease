...
###############################################################################
#
# Run Cox Proportional-Hazards models with trait-specific PGS tertiles  by EA
# level, sex (except for breast and prostate cancer), birth decade, and the first
# 10 genetic PCS as covariates.
#
################################################################################
...
write.table(modcoeffs.cox.model1a.Group1, file=paste0("[PathToOutputFolder/]",as.character(Sys.Date()),
                                                      "_",Biobank,"_INTERVENE_EducationalAttainment_CoxPH_model5_Coeffs_PGSGroup1.txt"),
...
write.table(modcoeffs.cox.model1a.Group2, file=paste0("[PathToOutputFolder/]",as.character(Sys.Date()),
                                                      "_",Biobank,"_INTERVENE_EducationalAttainment_CoxPH_model5_Coeffs_PGSGroup2.txt"),
            row.names=F, col.names = T, sep="\t",quote = F)
...
write.table(modcoeffs.cox.model1a.Group3, file=paste0("[PathToOutputFolder/]",as.character(Sys.Date()),
                                                      "_",Biobank,"_INTERVENE_EducationalAttainment_CoxPH_model5_Coeffs_PGSGroup3.txt"),
            row.names=F, col.names = T, sep="\t",quote = F)
